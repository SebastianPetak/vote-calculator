	...
	function addVotesFn() {
		...
		for (var i = 1; i <= numOfChoices; i++) {
			...
			choiceArray.push(entryValue);   // REMOVE?
		...
		for(var i = 1; i <= numOfChoices; i++) {
			for(var j = i + 1; j <= numOfChoices; j++) {
				pairArray.push(new pair(candidateNodeArray[i], candidateNodeArray[j]));
			}
		}
		addSingleVoteFn();
...
function voteCalc() {
	...
	* @return {Array of node objects} sourceList
	*/
	...
	this.calcResults = function(pairs, nodes) {
		...
		/* Fills candidatePairs array. */

		for(var i = 0; i < pairs.length; i++) {
			if((marginIndex = this.isContained(pairs[i].margin, candidatePairs)) >= 0) {
		...
		/* Sorts candidatePairs array by margin of victory. */
		...
		// NOTE:  Most efficient way to detect cycle?
...
/**
* Pair of candidates object.
*/

function pair(canNod1, canNod2) {

	if(!(this instanceof pair))
		return new pair(canNode1, canNode2);
	...
	*/
	...
	this.adjustMargin = function(cand) {
		(cand.indexID === this.cand1Node.indexID) ? this.margin++ : this.margin--;
		return true;
	}
...
function candNode(candName, indID) {
	...
	if(!(this instanceof candNode))
		return new candNode(candName, indID);
